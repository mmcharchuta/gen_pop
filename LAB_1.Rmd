...
boxplot_Ar_per_species <- ggplot(df_long, aes(x = reorder(species, AR), y = AR, fill = species)) +
  geom_boxplot() +
  labs(x = "Species", y = "Allelic Richness") +
  scale_fill_manual(values = RColorBrewer::brewer.pal(length(unique(df_long$species)), "Set3")) + 
  theme_classic()
print(boxplot_Ar_per_species)
...
barplot_Hs <- ggplot(basic_stats_df, aes(x= reorder(pop, HS), 
                                                y= HS, fill = species))+geom_bar(stat="identity")
barplot_Hs

barplot_Hs + scale_fill_manual(values = c("green4", "yellow2", "brown1", "dodgerblue")) + theme_classic()
...
barplot_Fis <- ggplot(basic_stats_df, aes(x= reorder(pop, FIS), 
                                                y= FIS, fill = species))+geom_bar(stat="identity")
barplot_Fis

barplot_Fis + scale_fill_manual(values = c("green4", "yellow2", "brown1", "dodgerblue")) + theme_classic()
...
```{r}
# zapisanie informacji o lokalizacji osobników jako ramka danych
loc <- as.data.frame(pines_data$other) 
# modyfikcja powyższej ramki by zawierała lokalizacje populacji
(hint – każdy osobnik w populacji ma tę samą lokalizację) 
loc_pops <- XXX(loc)
# stworzenie ramki danych zawierającej lokalizacje populacji, miary zmienności i oznaczenie gatunkowe
map_df <- data.frame( lat =loc$xy.x, long =loc$xy.y,species =… , Ho =…, He =…, Fis =…)
# przygotowanie mapy 
world <- ne_countries(scale = “medium”, returnclass = “sf”)
# wizualizacja pojedynczej miary zmienności, gatunek za pomocą kształtu, miara kolorem
ggplot(data = world) +
geom_sf() +
geom_point(data = map_df, aes(x = long, y = lat, shape = species, color = Ho), size = 4) +
coord_sf(xlim = c(-10, 40), ylim = c(40, 70), expand = FALSE)+
scale_shape_manual(values=c( 15, 16, 17,18))+
scale_color_continuous(type = “viridis”)+
theme_minimal()
# analogiczną wizualizację wykonaj dla pozostałych statystyk
